...
| src/Aetherphone/Windows/Components/ChatLineView.cs | Log layout renderer: one line per message with the game's own colors |
| src/Aetherphone/Windows/Components/Chat/ChatListChrome.cs | Shared list chrome (rows, headers, cards, setting rows) for ChocoChat and Linkpearl |
| src/Aetherphone/Windows/Components/Chat/ChatThemes.cs | The nine chat themes shared by ChocoChat and Linkpearl, with their inks and bubble styles |
| src/Aetherphone/Windows/Components/Chat/ChatWallpapers.cs | Colour and photo wallpapers behind a thread, global or per conversation |
| src/Aetherphone/Core/GameChat/GameLogColors.cs | Reads the game's own channel colours out of `UiConfig` for the import row |
| src/Aetherphone/Core/GameChat/GameChatMode.cs | Reads the game chat box's current channel so a tab can follow it |
| src/Aetherphone/Core/GameChat/NameMask.cs | Screenshot mode: stable pseudonyms for player names |
| src/Aetherphone/Core/GameChat/PopoutPlacement.cs | Where a fresh pop-out lands: beside the phone or in a chosen corner |
...
- The **in-game stack**: the Linkpearl app renders real game chat captured through Dalamud's `IChatGui`. It has no server and no reactions, so it does not use `ChatThreadStoreBase` or `ChatThreadView`. It has its own screen, `GameChatThread`, which reuses `ChatTranscript` for the Bubbles layout and its own `ChatLineView` for the Log layout.
...
- **Rendering.** `GameChatThread` is the whole screen below the header. Every conversation has two layouts, Log and Bubbles. A tab stores its own (`ChatTab.Density`, seeded from `Configuration.LinkpearlDefaultDensity` when the tab is created, plus `TextScale` and `Timestamps` overrides) and a tell stores its own in `Configuration.LinkpearlTellLayouts` through `TellPreferences.Layout` (Bubbles until changed); `InboxRow.Density` is the effective value and `ChatInbox.SetDensity` writes to whichever store owns the row. The thread header's layout button and the pop-out settings panel both flip it with one tap; `GameChatThread.Density` lets a screen notice the change and re-open the same key with the new target, which `GameChatThread.Open` treats as a target swap rather than a fresh conversation. Log is `ChatLineView`: one line per message in the game's own channel colours, with the timestamp and the sender as a clickable name (no underline until hovered, like the game), and the whole line drawn with the text size from `LinkpearlTextSize`. The colour alone says which channel a line came from, as in the game and Chat 2; `GameChannels.ShortCode` only labels lines in the history export. Bubbles is the shared `ChatTranscript` with the chat theme's bubble style and full sender names. Both draw text through `RunText`, which lays out styled runs with wrapping and hit testing so every game link is tappable. `GameChatTargets.For(row)` turns an `InboxRow` into the `GameChatTarget` the thread opens, so the app screen and the pop-out windows open conversations the same way. A stream can hold `ChatLog.MaxLinesPerStream` (2000) lines, so the thread only draws a window over the tail: `WindowPageLines` (100) visible lines to start, growing by a page whenever the user scrolls within `LoadOlderThreshold` of the top. Bubbles get this through the transcript's `IChatTranscriptPaging` hook (the thread is its own synchronous pager); the Log layout grows the window in its own loop and keeps the viewport still by measuring where last frame's first line landed in content space, then shifting `ScrollY` by exactly that much in the same frame. Growth waits while the kinetic scroller is dragging, flinging, or rubber-banding, because `KineticScroller.SyncOffset` ignores external scroll changes in those states. `Reveal` (search hits, deep links) widens the window to include its target before either path draws.
- **Read state.** A watermark per conversation, persisted in `Configuration.LinkpearlSeen`, advanced only while the conversation is actually on screen. `ChatInbox.IsViewing` is true for the conversation the app's thread screen noted this frame (`NoteViewing`, backed by the same `ViewingMark` the Aethernet chat stores use, so it lapses 4 seconds after drawing stops and a closed phone stops counting as viewing) and for any expanded pop-out that is hovered or focused (`SetAttended`); both the unread counter and `ChatNotifier` consult it. The thread screen follows the same clear-on-view contract as `ChatThreadView`: every frame it is on screen, `LinkpearlApp.NoteConversationViewed` notes the key, marks the row read if it has unread, and removes the row's notification group, so unread that accrued while the app was backgrounded clears on resume without a trip back to the list. A collapsed pop-out never counts as viewing, so unread keeps climbing behind the badge on its bar even while the pointer rests on it. There are no receipts because the game has none.
...
- **The app shell.** `LinkpearlApp` draws bezel to bezel on `SceneChrome.AppAreaFrom` with the shared `ChatListChrome` (the same rows, headers and cards ChocoChat uses) and three bottom tabs: Chats, People and Settings. Its colours come from a chat theme (`ChatThemes`, stored in `Configuration.LinkpearlChatTheme`, defaulting to the green Leaf theme that matches the home tile) rather than a fixed palette, so the accent, sent bubbles, badges and chrome inks all follow the picked theme. `LinkpearlRoute` names every sub-screen (conversation, tab editor, tab info, a settings section, the theme and wallpaper pickers, and the friend, character and free company cards). `DrawView` paints `ui.Body(area)` before any screen, the way Aethergram and ChocoChat do, because the router's layers carry a transparent background and a push would otherwise show the outgoing screen through the incoming one.
- **The Chats tab.** A header (title, new chat, more), a search field, a paused-notifications banner, a filter `ChipRail` (All, Tells, Tabs, Unread, shown once there are four or more conversations), then pinned rows and the rest by activity. `InboxRowView` reveals pin, mute and more buttons on hover and returns an `InboxRowAction`; right-click opens the same `conversationMenu` the thread header uses. New tabs come from the `SheetSurface` behind the plus button (preset tiles for Free Company, Linkshells, Party and Local drawn with the same `GameChatTiles` tile the row will show, then a card with Custom tab and Send a tell), skinned through `SheetSkin.From(palette, ink)` so the panel follows the chat theme, so nothing lives at the bottom of the list any more.
- **The People tab.** Friends, the party and the free company as `FriendRow`s with a portrait, an online dot and a subtitle that wraps instead of truncating; a row opens the friend card, and the header search finds any character through Lodestone. The character and free company cards are the Lodestone screens.
- **The Settings tab.** Grouped rows on `ChatListChrome` cards: Appearance (chat theme, wallpaper, layout, text size, timestamps, the Log layout switches), Notifications, Pop-outs (tabs, placement, opacity, fade, flash, tells), Behavior (hide in cutscenes and with the HUD), Channels (colours, rules, and an Import game colours row that reads `GameLogColors`), History (retention, export through `ChatArchive.Export`), and Privacy (screenshot mode, which routes every player name through `NameMask`). A tab can also follow the game chat box's current channel through `GameChatMode` (`GameChannels.FollowGameKey`).
- **Themes and wallpapers.** `ChatThemes` and `ChatWallpapers` are shared with ChocoChat and picked through `ChatAppearancePickers`; the wallpaper is global (`LinkpearlWallpaper`) with per conversation overrides (`LinkpearlChatWallpapers`, resolved by `ChatWallpapers.Effective`) and an optional pattern, painted behind the transcript through `GameChatThread.Backdrop`.
- **Pop-out windows.** `LinkpearlPopouts` (src/Aetherphone/Windows/LinkpearlPopouts.cs) owns a pool of `LinkpearlPopoutWindow` Dalamud windows, one conversation each, registered in the plugin's `WindowSystem` next to the phone. They draw their own glass surface in the app's chat theme (`ChatThemes.Body`, the theme accent tinting the strip, `SocialInk` for strokes and inks, the wallpaper painted behind the transcript at the window's opacity), a title strip that is the tab rail (one chip per conversation with its tile or portrait, name and unread pill; the active chip opens the switch menu, a plus chip adds a tab, and the right end holds settings, collapse, open-in-phone and close), and a `GameChatThread`, and they keep working while the phone is closed or minimized. The settings button (`LinkpearlPopoutWindow.Settings.cs`) swaps the thread for a panel of grouped cards drawn with the same `GroupCard` and `SettingsRow` kit as the phone's settings sections: this chat's layout as a Log or Bubbles segment, the pop-out text size, opacity and fade, and the Log switches while the chat is a log (Timestamps edits the tab's override when the tab editor set one, otherwise the global). The panel closes on tab switch, collapse and unbind, and its scroll child is pushed under its own id so it never shares scroll state with the Log layout's child. Placement is persisted in `Configuration.LinkpearlPopouts` and restored on load. Closing a window (the close button, the last tab closed, or Close all pop-outs) remembers its position and expanded size in `Configuration.LinkpearlPopoutLastPlacement`, and the next fresh window opens there through `PopoutPlacements.Recall` (clamped to the viewport, staggered per other open window). With nothing remembered, or after the placement setting changes (which clears the memory), a fresh window lands where `PopoutPlacements.Resolve` puts it (beside the phone by default, or the corner picked in `LinkpearlPopoutPlacement`, staggered per other open window), using the phone frame that `PhoneVisibility` probes each frame. Opacity and text size come from `LinkpearlPopoutOpacity` / `LinkpearlPopoutTextScale`. With `LinkpearlPopoutTells` on, a new tell pops its own window automatically when the phone is not visible, unless notifications are paused, Do Not Disturb is on, or the tell is muted. A pop-out only marks its conversation read while it is hovered or focused, so an auto-opened window keeps its unread pill until the player looks at it. The context menu is the shared `GameChatMenu`; actions that need the phone (look up, market) hand off through `LinkpearlLauncher` and `Plugin.ShowPhoneApp`. Confirms are the exception: a window reserves a `ConfirmHosts` id, wraps its draw in `ConfirmHosts.Enter`, and draws its own `ConfirmOverlay` over its frame, so opening a link from a pop-out asks in that window rather than on a phone the player may have minimized. The frame is shielded while the card is up, Escape cancels it, and collapsing, unbinding, or suppressing the window drops the request through `ConfirmService.CancelHost`. Each window sets `UiScale.SetPhone` and `Plugin.Fonts.SetPhoneZoom` in `PreDraw` like `PhoneWindow` does, and disables `DragScrollHost` so the transcript scrolls with the wheel like a desktop window.
- **Grouping pop-outs as tabs.** A window holds up to six conversations: `LinkpearlPopoutState` carries `Keys` and `Active`, migrated lazily from the old single `Key` on restore, and `PopoutTabs` holds the pure list maths. The title strip is the tab rail: when the chips no longer fit, every chip but the active one shrinks to its tile with the name on hover. A collapsed window shows the stacked tab avatars, the joined names and one "N new" pill for the whole group. Only the active tab is attended, so background tabs keep accruing unread. One `GameChatThread` per window is re-targeted on switch rather than one per tab, because each thread owns a `ChatStreamView` that subscribes to the log and merges hundreds of entries. With `LinkpearlPopoutTabs` on (the default) a new conversation joins the most recently active window that has room and only opens a fresh window when every window is full; with it off every conversation gets its own window. Dragging a pop-out over another merges them, and the menu on the active chip can move a tab out into its own window.
