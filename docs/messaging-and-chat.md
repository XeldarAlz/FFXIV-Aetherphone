...
- **The app shell.** `LinkpearlApp` draws bezel to bezel on `SceneChrome.AppAreaFrom` with the shared `ChatListChrome` (the same rows, headers and cards ChocoChat uses) and three bottom tabs: Chats, People and Settings. Its colours come from a chat theme (`ChatThemes`, stored in `Configuration.LinkpearlChatTheme`, defaulting to the green Leaf theme that matches the home tile) rather than a fixed palette, so the accent, sent bubbles, badges and chrome inks all follow the picked theme. `LinkpearlRoute` names every sub-screen (conversation, tab editor, tab info, a settings section, the theme and wallpaper pickers, and the friend, character and free company cards).
