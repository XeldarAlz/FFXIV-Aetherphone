...
- `Installed` decides which apps exist on the phone. First run seeds it with every available app; installing via the App Store app (`AppStoreApp`, id `"appstore"`) appends a tile to the last page. `MandatoryApps` (`"appstore"`, `"settings"`, `"announcements"`, `"messages"` for Linkpearl, `"camera"`, `"photos"`) cannot be uninstalled, show no remove badge in edit mode, and are left out of the App Store listing.
