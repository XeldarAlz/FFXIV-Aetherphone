...
internal sealed class VelvetPostComposer
{
    ...
    private void DrawCrop(Rect area, VelvetUi ui, in PhoneContext context)
    {
        ...
        var stage = new Rect(new Vector2(area.Min.X + 16f * scale, top + 12f * scale), new Vector2(area.Max.X - 16f * scale, area.Max.Y - 240f * scale));
        ...
        if (texture is null)
        {
            Squircle.Fill(drawList, preview.Min, preview.Max, rounding, ImGui.GetColorU32(new Vector4(1f, 1f, 1f, 0.10f)));
        ...
        Typography.DrawCentered(new Vector2(area.Center.X, hintY), Loc.T(L.Velvet.GestureHint), VelvetUi.MutedInk, 0.78f);

        var trackWidth = area.Width * 0.62f;
        var trackY = hintY + 22f * scale;
        var track = new Rect(new Vector2(area.Center.X - trackWidth * 0.5f, trackY), new Vector2(area.Center.X + trackWidth * 0.5f, trackY + 4f * scale));
