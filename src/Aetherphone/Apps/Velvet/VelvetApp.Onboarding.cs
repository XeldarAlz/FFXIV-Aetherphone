...
internal sealed partial class VelvetApp
{
    private void DrawOnboarding(Rect content)
    {
        ...
        Typography.DrawCentered(new Vector2(centerX, content.Min.Y + 60f * scale), title, VelvetUi.TitleInk, 1.5f, FontWeight.SemiBold);
        ...
        if (onboardStep > 0)
        {
            ...
            if (ui.GhostButton(new Rect(backMin, new Vector2(backMin.X + buttonWidth, backMin.Y + buttonHeight)), Loc.T(L.Velvet.Back)))
            {
                onboardStep--;
            }
        }
    ...
    private void DrawGate(Rect content)
    {
        ...
        drawList.AddCircleFilled(moonCenter, 46f * scale, ImGui.GetColorU32(new Vector4(0.90f, 0.24f, 0.46f, 0.16f)), 40);
        drawList.AddCircleFilled(moonCenter, 30f * scale, ImGui.GetColorU32(new Vector4(0.95f, 0.42f, 0.60f, 0.22f)), 36);
        VelvetUi.Icon(moonCenter, FontAwesomeIcon.Moon.ToIconString(), new Vector4(0.99f, 0.72f, 0.82f, 1f), 1.7f);

        Typography.DrawCentered(new Vector2(centerX, moonCenter.Y + 66f * scale), Loc.T(L.Velvet.GateTitle), VelvetUi.TitleInk, 1.7f, FontWeight.SemiBold);

        var bodyTop = moonCenter.Y + 96f * scale;
        var bodyHeight = VelvetUi.WrappedCentered(centerX, bodyTop, Loc.T(L.Velvet.GateBody), textWidth, VelvetUi.BodyInk, scale, 1.05f);
        VelvetUi.WrappedCentered(centerX, bodyTop + bodyHeight + 16f * scale, Loc.T(L.Velvet.GateDiscretion), textWidth, VelvetUi.MutedInk, scale, 0.88f);
        ...
        if (ui.PillButton(enterRect, gateBusy ? Loc.T(L.Velvet.GateWorking) : Loc.T(L.Velvet.GateEnter), true) && !gateBusy)
        {
            AcceptGate();
        }
