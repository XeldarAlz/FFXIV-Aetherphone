...
internal sealed class AethergramApp : IPhoneApp
{
    ...
    private void DrawGramCard(PostDto post)
    {
        ...
        if (DrawIconButton(moreCenter, 12f * scale, FontAwesomeIcon.EllipsisH.ToIconString(), AethergramUi.BodyInk,
                AethergramUi.Transparent, 0.85f, Loc.T(L.Aethergram.More)))
        {
        ...
        if (DrawIconButton(heartCenter, 14f * scale, FontAwesomeIcon.Heart.ToIconString(),
                liked ? LikeRed : AethergramUi.BodyInk, AethergramUi.Transparent, 1.15f, Loc.T(L.Aethergram.Like)))
        {
        ...
        if (post.TotalReactions > 0)
        {
            ...
            Typography.Draw(new Vector2(cursorX, actionCenterY - 7f * scale), likeText, AethergramUi.BodyInk, 0.82f,
                FontWeight.Medium);
        ...
        if (DrawIconButton(commentCenter, 14f * scale, FontAwesomeIcon.Comment.ToIconString(), AethergramUi.BodyInk,
                AethergramUi.Transparent, 1.05f, Loc.T(L.Aethergram.Comment)))
        {
        ...
        if (post.CommentCount > 0)
        {
            Typography.Draw(new Vector2(commentCenter.X + 18f * scale, actionCenterY - 7f * scale),
                post.CommentCount.ToString(Loc.Culture), AethergramUi.BodyInk, 0.82f, FontWeight.Medium);
        }
    ...
    private void DrawLikeBurst(Rect imageRect, string postId)
    {
        ...
        DrawIcon(center + new Vector2(0f, 2f * scale), FontAwesomeIcon.Heart.ToIconString(),
    ...
    private void DrawGramImage(Rect rect, string? url, float rounding)
    {
        ...
        if (texture is null)
        {
            ...
            Typography.DrawCentered(rect.Center,
                Loc.T(images.Failed(url) ? L.Aethergram.ImageFailed : L.Common.Loading), AethergramUi.MutedInk, 0.85f);
            return;
        }
    ...
    private void DrawBottomNav(Rect bar)
    {
        ...
        if (DrawIconButton(homeCenter, hitRadius, FontAwesomeIcon.Home.ToIconString(), AethergramUi.TitleInk, none,
                1.15f))
        {
        ...
        if (DrawIconButton(searchCenter, hitRadius, FontAwesomeIcon.Search.ToIconString(), AethergramUi.MutedInk, none,
                1.1f))
        {
            store.ClearDiscover();
            searchDraft = string.Empty;
        ...
        if (DrawIconButton(postCenter, hitRadius, FontAwesomeIcon.PlusSquare.ToIconString(), AethergramUi.MutedInk,
                none, 1.2f))
        {
            StartCompose(false);
        }
        ...
        {
            store.EnsureMe();
            DrawIconButton(profileCenter, hitRadius, FontAwesomeIcon.User.ToIconString(), AethergramUi.MutedInk,
                AethergramUi.Transparent, 1.05f);
        }
    }
        ...
        using (AppSurface.Begin(gridRect))
        {
            if (pickerPaths.Length == 0)
            {
                Typography.DrawCentered(new Vector2(gridRect.Center.X, gridRect.Min.Y + 60f * scale),
                    Loc.T(L.Photos.NoPhotos), AethergramUi.MutedInk);
                return;
            }
    ...
    private bool DrawShareBar(Rect rect, string label, bool enabled)
    {
        ...
        DrawIcon(new Vector2(left + iconWidth * 0.5f, rect.Center.Y), FontAwesomeIcon.PaperPlane.ToIconString(), ink,
            0.9f);
    ...
    private void DrawDetail(Rect area, string postId)
    {
        ...
        using (AppSurface.Begin(body))
        {
            ...
            var nameLeft = avatarCenter.X + avatarRadius + 12f * scale;
            var displayName = string.IsNullOrEmpty(post.AuthorDisplayName) ? post.AuthorName : post.AuthorDisplayName;
            Typography.Draw(new Vector2(nameLeft, avatarCenter.Y - 14f * scale), displayName, theme.TextStrong, 0.95f,
                FontWeight.SemiBold);
            Typography.Draw(new Vector2(nameLeft, avatarCenter.Y + 2f * scale),
                $"{post.AuthorWorld} · {RelativeTime(post.CreatedAtUnix)}", AethergramUi.MutedInk, 0.78f);
            ...
            if (DrawIconButton(heartCenter, 14f * scale, FontAwesomeIcon.Heart.ToIconString(),
                    liked ? LikeRed : AethergramUi.BodyInk, AethergramUi.Transparent, 1.1f, Loc.T(L.Aethergram.Like)))
            {
                store.ToggleLike(post);
            }
            ...
            {
                var likeText = post.TotalReactions.ToString(Loc.Culture);
                Typography.Draw(new Vector2(actionCursorX, actionsY - 7f * scale), likeText, AethergramUi.BodyInk,
            ...
            if (post.CommentCount > 0)
            {
                Typography.Draw(new Vector2(commentCenter.X + 18f * scale, actionsY - 7f * scale),
                    post.CommentCount.ToString(Loc.Culture), AethergramUi.BodyInk, 0.85f, FontWeight.Medium);
            }
            ...
            {
                Typography.Draw(ImGui.GetCursorScreenPos(), Loc.T(L.Aethergram.NoComments), AethergramUi.MutedInk,
                    0.85f);
            }
            else
            {
    ...
    private void DrawComment(CommentDto comment)
    {
        ...
        if (mine)
        {
            ...
            if (ui.IconButton(trashCenter, trashHitRadius, FontAwesomeIcon.Times.ToIconString(), AethergramUi.MutedInk,
                    AethergramUi.Transparent, 0.7f) && store.DetailPost is { } post)
            {
    ...
    private void DrawCommentComposer(Rect bar, string postId)
    {
        ...
        Squircle.Fill(drawList, pillMin, pillMax, (pillMax.Y - pillMin.Y) * 0.5f,
            ImGui.GetColorU32(AethergramUi.FieldSurface));
        ...
        if (posts.Length == 0)
        {
            Typography.DrawCentered(
                new Vector2(ImGui.GetCursorScreenPos().X + ImGui.GetContentRegionAvail().X * 0.5f,
                    ImGui.GetCursorScreenPos().Y + 40f * scale), Loc.T(L.Aethergram.Empty), AethergramUi.MutedInk);
            return;
        }
    ...
    private void DrawSearchBar(Rect bar)
    {
        ...
        DrawIcon(new Vector2(pillMin.X + 16f * scale, (pillMin.Y + pillMax.Y) * 0.5f),
            FontAwesomeIcon.Search.ToIconString(), AethergramUi.MutedInk, 0.85f);
    ...
    private void DrawHomeTopBar(Rect area)
    {
        ...
        Typography.Draw(new Vector2(area.Min.X + 16f * scale, rowCenterY - logoSize.Y * 0.5f), DisplayName,
            AethergramUi.TitleInk, 1.3f, FontWeight.Bold);
    }
    ...
    private void DrawHandleField()
    {
        ...
        Typography.Draw(new Vector2(origin.X + 2f * scale, origin.Y + height + 3f * scale),
            Loc.T(L.Aethergram.HandleRules), AethergramUi.MutedInk, 0.78f);
