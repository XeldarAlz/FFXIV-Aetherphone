...
internal sealed class AccountPage : ISettingsPage, IDisposable
{
    ...
    private static void DrawIdentityCard(string name, string world, PhoneTheme theme)
    {
        ...
        Typography.Draw(new Vector2(start.X + padding, start.Y + padding + labelSize.Y + 4f * scale), identity, theme.TextStrong, 1.05f, FontWeight.SemiBold);
    ...
    private bool DrawCodeCard(PhoneTheme theme)
    {
        ...
        Squircle.Stroke(drawList, start, max, radius, ImGui.GetColorU32(Palette.WithAlpha(theme.Accent, 0.55f)), 1.4f * scale);
