...
internal sealed class SnakeRenderer
{
    ...
    public void Draw(SnakeBoard board, Rect area, float scale)
    {
        ...
        Squircle.Stroke(drawList, area.Min, area.Max, 16f * scale, ImGui.GetColorU32(new Vector4(1f, 1f, 1f, 0.06f)), 1.4f * scale);
    ...
    private void DrawFood(ImDrawListPtr drawList, SnakeBoard board, Rect area)
    {
        ...
        drawList.AddCircleFilled(board.Food - new Vector2(radius * 0.3f, radius * 0.3f), radius * 0.34f, ImGui.GetColorU32(new Vector4(1f, 1f, 1f, 0.6f)), 16);
    }
