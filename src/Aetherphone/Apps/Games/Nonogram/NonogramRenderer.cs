...
internal sealed class NonogramRenderer
{
    ...
    public void Draw(NonogramBoard board, NonogramLayout layout, int hoveredCell, float[] fillAnimation, PhoneTheme theme, Vector4 accent, float scale)
    {
        ...
        for (var row = 0; row < board.Size; row++)
        {
            for (var column = 0; column < board.Size; column++)
            {
                ...
                DrawCell(drawList, board.MarkAt(index), min, max, inset, rounding, index == hoveredCell, fillAnimation[index], accent, scale);
            }
        }
    ...
    private void DrawCell(ImDrawListPtr drawList, CellMark mark, Vector2 min, Vector2 max, float inset, float rounding, bool hovered, float fillPop, Vector4 accent, float scale)
    {
        ...
        if (mark == CellMark.Filled)
        {
            ...
            Squircle.Fill(drawList, fillMin, new Vector2(fillMax.X, fillMin.Y + half.Y), rounding, ImGui.GetColorU32(GamePalette.Lighten(accent, 0.22f) with { W = 0.5f }));
            return;
        }
    ...
    private void DrawColumnClues(ImDrawListPtr drawList, NonogramBoard board, NonogramLayout layout, Vector4 accent, PhoneTheme theme, float clueScale, float cell)
    {
        for (var column = 0; column < board.Size; column++)
        {
            ...
            for (var slot = 0; slot < count; slot++)
            {
                ...
                Typography.DrawCentered(new Vector2(centerX, centerY), GameNumber.Label(board.ColumnClue(column, slot)), theme.TextStrong, clueScale, FontWeight.SemiBold);
            }
        }
    }

    private void DrawRowClues(ImDrawListPtr drawList, NonogramBoard board, NonogramLayout layout, Vector4 accent, PhoneTheme theme, float clueScale, float cell)
    {
        for (var row = 0; row < board.Size; row++)
        {
            ...
            for (var slot = 0; slot < count; slot++)
            {
                ...
                Typography.DrawCentered(new Vector2(centerX, centerY), GameNumber.Label(board.RowClue(row, slot)), theme.TextStrong, clueScale, FontWeight.SemiBold);
            }
        }
    }

    private void DrawGridLines(ImDrawListPtr drawList, NonogramBoard board, Vector2 gridMin, Vector2 gridMax, float cell, float scale)
    {
