...
internal sealed class BreakoutRenderer
{
    ...
    public void Draw(BreakoutBoard board, Rect field, Vector4 accent, float scale)
    {
        ...
        for (var row = 0; row < board.Rows; row++)
        {
            for (var column = 0; column < BreakoutBoard.Columns; column++)
            {
                ...
                Squircle.Fill(drawList, min, new Vector2(max.X, center.Y), rounding,
                    ImGui.GetColorU32(new Vector4(1f, 1f, 1f, 0.16f)));
            }
        }
        ...
        for (var index = 0; index < board.PowerUpCount; index++)
        {
            ...
            Typography.DrawCentered(center, glyph, new Vector4(0.1f, 0.12f, 0.14f, 1f), radius / (9f * scale),
                FontWeight.Bold);
        }
        ...
        Squircle.Fill(drawList, paddleMin, new Vector2(paddleMax.X, paddleCenter.Y), paddleHalf.Y,
