...
internal sealed class TetrisApp : IMiniGame
{
    ...
    public void Draw(in GameContext context)
    {
        ...
        var holdRect = new Rect(new Vector2(body.Min.X + 12f * scale, slotTop), new Vector2(body.Min.X + 12f * scale + slotSize, slotTop + slotSize));
        var nextRect = new Rect(new Vector2(body.Max.X - 12f * scale - slotSize, slotTop), new Vector2(body.Max.X - 12f * scale, slotTop + slotSize));
        ...
        GameHud.Pill(new Vector2(body.Center.X - 44f * scale, body.Min.Y + 32f * scale), Loc.T(L.Games.Score), GameNumber.Label(board.Score), Accent, theme);
        GameHud.Pill(new Vector2(body.Center.X + 44f * scale, body.Min.Y + 32f * scale), Loc.T(L.Games.Best), GameNumber.Label(bestScore), Accent, theme, bestScore > 0 && board.Score < bestScore);
        GameHud.Pill(new Vector2(body.Center.X - 30f * scale, body.Min.Y + 82f * scale), Loc.T(L.Games.Level), GameNumber.Label(board.Level), Accent, theme);

        if (GameHud.RestartButton(new Vector2(body.Center.X + 52f * scale, body.Min.Y + 82f * scale), 15f * scale, theme))
        {
            StartGame();
            return;
        }
        ...
        var field = new Rect(new Vector2(body.Min.X + 12f * scale, body.Min.Y + 114f * scale), new Vector2(body.Max.X - 12f * scale, body.Max.Y - 52f * scale));
    ...
    private static void DrawMoveIcon(ImDrawListPtr drawList, Vector2 center, int direction, float scale, uint color)
    {
        ...
        drawList.AddTriangleFilled(
            ...
            new Vector2(center.X - direction * size * 0.6f, center.Y + size),
            new Vector2(center.X + direction * size, center.Y),
            color);
    }
    ...
    private static void DrawRotateIcon(ImDrawListPtr drawList, Vector2 center, float scale, uint color)
    {
        ...
        drawList.AddTriangleFilled(
            tip + tangent * head,
            tip - tangent * head * 0.2f + normal * head * 0.6f,
            tip - tangent * head * 0.2f - normal * head * 0.6f,
            color);
    }
    ...
    private static void DrawSwapIcon(ImDrawListPtr drawList, Vector2 center, float scale, uint color)
    {
        ...
        drawList.AddLine(new Vector2(center.X - width, topY), new Vector2(center.X + width * 0.3f, topY), color, thickness);
        drawList.AddTriangleFilled(
            new Vector2(center.X + width * 0.2f, topY - head),
            new Vector2(center.X + width * 0.2f, topY + head),
            new Vector2(center.X + width, topY),
            color);

        var bottomY = center.Y + offset;
        drawList.AddLine(new Vector2(center.X + width, bottomY), new Vector2(center.X - width * 0.3f, bottomY), color, thickness);
        drawList.AddTriangleFilled(
            new Vector2(center.X - width * 0.2f, bottomY - head),
            new Vector2(center.X - width * 0.2f, bottomY + head),
            new Vector2(center.X - width, bottomY),
            color);
    }

    private static void DrawDropIcon(ImDrawListPtr drawList, Vector2 center, float scale, uint color)
    {
        drawList.AddLine(new Vector2(center.X, center.Y - 5.5f * scale), new Vector2(center.X, center.Y + 1f * scale), color, 1.8f * scale);
        drawList.AddTriangleFilled(
            new Vector2(center.X - 3.2f * scale, center.Y + 0.5f * scale),
            new Vector2(center.X + 3.2f * scale, center.Y + 0.5f * scale),
            new Vector2(center.X, center.Y + 4.5f * scale),
            color);
        drawList.AddRectFilled(new Vector2(center.X - 4.5f * scale, center.Y + 5.4f * scale), new Vector2(center.X + 4.5f * scale, center.Y + 6.8f * scale), color);
    }
