...
internal sealed class FlapRenderer
{
    ...
    private void DrawSky(ImDrawListPtr drawList, Rect area, float scale)
    {
        ...
        {
            var baseX = area.Min.X + ((index * 0.42f * area.Width + drift) % (area.Width + 0.4f * area.Width)) - 0.2f * area.Width;
    ...
    private void DrawPipes(ImDrawListPtr drawList, FlapBoard board, Rect area, float scale)
    {
        ...
        for (var index = 0; index < board.PipeCount; index++)
        {
            ...
            DrawPipeSegment(drawList, new Vector2(left, area.Min.Y), new Vector2(right, gapTop), body, edge, sheen, rounding, scale);
            DrawPipeSegment(drawList, new Vector2(left, gapBottom), new Vector2(right, area.Max.Y), body, edge, sheen, rounding, scale);

            var capRounding = capHeight * 0.4f;
            drawList.AddRectFilled(new Vector2(left - capOverhang, gapTop - capHeight), new Vector2(right + capOverhang, gapTop), body, capRounding);
            drawList.AddRectFilled(new Vector2(left - capOverhang, gapBottom), new Vector2(right + capOverhang, gapBottom + capHeight), body, capRounding);
            drawList.AddRect(new Vector2(left - capOverhang, gapTop - capHeight), new Vector2(right + capOverhang, gapTop), edge, capRounding, ImDrawFlags.RoundCornersAll, 1.4f * scale);
            drawList.AddRect(new Vector2(left - capOverhang, gapBottom), new Vector2(right + capOverhang, gapBottom + capHeight), edge, capRounding, ImDrawFlags.RoundCornersAll, 1.4f * scale);
        }
    }

    private void DrawPipeSegment(ImDrawListPtr drawList, Vector2 min, Vector2 max, uint body, uint edge, uint sheen, float rounding, float scale)
    {
        if (max.Y <= min.Y)
        {
            return;
        ...
        drawList.AddRectFilled(new Vector2(min.X + (max.X - min.X) * 0.16f, min.Y), new Vector2(min.X + (max.X - min.X) * 0.34f, max.Y), sheen, rounding);
    ...
    private void DrawBird(ImDrawListPtr drawList, float x, float y, float radius, float tilt, float scale)
    {
        ...
        drawList.AddCircleFilled(Rotate(center, new Vector2(radius * 0.5f, -radius * 0.32f), tilt), radius * 0.12f, ImGui.GetColorU32(new Vector4(0.1f, 0.1f, 0.12f, 1f)), 12);
