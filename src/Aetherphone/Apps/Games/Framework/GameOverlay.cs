...
internal static class GameOverlay
{
    public static bool Draw(Rect area, PhoneTheme theme, Vector4 accent, float progress, in GameResult result)
    {
        ...
        Typography.DrawCentered(new Vector2(center.X, top + 36f * scale), result.Title,
            result.TitleColor with { W = result.TitleColor.W * textAlpha }, TextStyles.Title1);
        if (result.NewBest)
        {
            ...
            Squircle.Fill(drawList, badgeCenter - badgeHalf, badgeCenter + badgeHalf, badgeHalf.Y,
                ImGui.GetColorU32(accent with { W = 0.22f * textAlpha }));
            Typography.DrawCentered(badgeCenter, badge, accent with { W = textAlpha }, TextStyles.FootnoteEmphasized);
        }

        Typography.DrawCentered(new Vector2(center.X, center.Y - 2f * scale), result.PrimaryValue,
            theme.TextStrong with { W = textAlpha }, TextStyles.LargeTitle);
        Typography.DrawCentered(new Vector2(center.X, center.Y + 26f * scale), result.PrimaryLabel.ToUpperInvariant(),
            theme.TextMuted with { W = textAlpha }, TextStyles.Caption1);
        if (!string.IsNullOrEmpty(result.SecondaryLine))
        {
            Typography.DrawCentered(new Vector2(center.X, center.Y + 48f * scale), result.SecondaryLine,
                theme.TextMuted with { W = textAlpha }, TextStyles.Footnote);
        }
