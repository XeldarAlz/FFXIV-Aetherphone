...
internal sealed class ReversiRenderer
{
        ...
        float scale)
    {
        ...
        Squircle.Fill(drawList, grid.Origin, new Vector2(grid.Origin.X + grid.Width, grid.Origin.Y + grid.Height), 6f * scale, ImGui.GetColorU32(Felt));
        ...
        for (var row = 0; row < ReversiBoard.Size; row++)
        {
            for (var column = 0; column < ReversiBoard.Size; column++)
            {
                ...
                DrawCell(drawList, board, index, center, radius, flipTimer[index], flipFrom[index], placeTimer[index], flipDuration, placeDuration, showHints, hintPlayer, accent, hintPulse, scale);
            }
        }
    }

    private void DrawCell(ImDrawListPtr drawList, ReversiBoard board, int index, Vector2 center, float radius, float flip, int flipFrom, float place, float flipDuration, float placeDuration, bool showHints, int hintPlayer, Vector4 accent, float hintPulse, float scale)
    {
        if (flip > 0f)
        {
    ...
    private void DrawDisc(ImDrawListPtr drawList, Vector2 center, float radius, float squashY, float popScale, int colorIndex, float scale)
    {
        ...
        Squircle.Fill(drawList, min + new Vector2(0f, 2f * scale), max + new Vector2(0f, 2f * scale), corner, ImGui.GetColorU32(new Vector4(0f, 0f, 0f, 0.28f)));
        Squircle.Fill(drawList, min, max, corner, ImGui.GetColorU32(body));
        Squircle.Fill(drawList, min, new Vector2(max.X, center.Y), corner, ImGui.GetColorU32(GamePalette.Lighten(body, 0.22f) with { W = 0.5f }));
        Squircle.Stroke(drawList, min, max, corner, ImGui.GetColorU32(GamePalette.Darken(body, 0.3f) with { W = 0.6f }), 1f * scale);
    }
    ...
    private void DrawGridLines(ImDrawListPtr drawList, GameGrid grid, float scale)
    {
        ...
        for (var line = 0; line <= ReversiBoard.Size; line++)
        {
            ...
            drawList.AddLine(new Vector2(x, grid.Origin.Y), new Vector2(x, grid.Origin.Y + grid.Height), color, 1f * scale);
            var y = grid.Origin.Y + line * grid.Pitch;
            drawList.AddLine(new Vector2(grid.Origin.X, y), new Vector2(grid.Origin.X + grid.Width, y), color, 1f * scale);
        }
    }
}
