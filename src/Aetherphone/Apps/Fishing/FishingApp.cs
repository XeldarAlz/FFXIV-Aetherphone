...
internal sealed class FishingApp : IPhoneApp
{
    ...
    private void DrawHero(PhoneTheme theme, DateTime utcNow, float scale)
    {
        ...
        var fraction = current.BoardingNow ? 1f : 1f - (float)Math.Clamp(remaining.TotalSeconds / VoyagePeriodSeconds, 0f, 1f);
        ...
        Typography.DrawCentered(new Vector2(centerX, ringCenter.Y + radius + 24f * scale), status.ToUpperInvariant(), tint, TextStyles.FootnoteEmphasized);

        var heading = $"{plan.RouteName} · {TimeOfDayLabel(plan.TimeOfDay)}";
        Typography.DrawCentered(new Vector2(centerX, ringCenter.Y + radius + 46f * scale), heading, theme.TextStrong, TextStyles.Title3);

        var when = current.BoardingNow ? Loc.T(L.Time.Now) : $"{LocalTime(current.BoardingUtc)} · {Relative(remaining)}";
        Typography.DrawCentered(new Vector2(centerX, ringCenter.Y + radius + 70f * scale), when, theme.TextMuted, TextStyles.Subheadline);
    ...
    private static void DrawBlueFish(PhoneTheme theme, in OceanRoutePlan plan, float scale)
    {
        ...
        if (plan.BlueFish.Length == 0)
        {
            ...
            Typography.Draw(new Vector2(row.Min.X, row.Center.Y - labelSize.Y * 0.5f), Loc.T(L.Fishing.NoBlueFish), theme.TextMuted, TextStyles.Subheadline);
            card.End();
            return;
        }
    ...
    private static void DrawBlueFishRow(Rect row, PhoneTheme theme, in OceanBlueFish fish, float scale)
    {
        ...
        Typography.Draw(new Vector2(textLeft, row.Center.Y - 16f * scale), fish.Name, theme.TextStrong, TextStyles.Headline);
        Typography.Draw(new Vector2(textLeft, row.Center.Y + 5f * scale), fish.Bait, theme.TextMuted, TextStyles.Footnote);
    }
    ...
    private static void DrawVoyageRow(Rect row, PhoneTheme theme, in OceanVoyageSlot voyage, DateTime utcNow, float scale)
    {
        ...
        var textLeft = row.Min.X + tile + 12f * scale;
        var heading = $"{plan.RouteName} · {TimeOfDayLabel(plan.TimeOfDay)}";
        Typography.Draw(new Vector2(textLeft, row.Center.Y - 19f * scale), heading, theme.TextStrong, TextStyles.Headline);
        ...
        Typography.Draw(new Vector2(row.Max.X - timeSize.X, row.Center.Y - 19f * scale), time, theme.TextStrong, TextStyles.SubheadlineEmphasized);

        var relative = Relative(voyage.BoardingUtc - utcNow);
        var relativeSize = Typography.Measure(relative, TextStyles.Caption1);
        Typography.Draw(new Vector2(row.Max.X - relativeSize.X, row.Center.Y + 4f * scale), relative, theme.TextMuted, TextStyles.Caption1);
    }
    ...
    private static Vector4 TimeOfDayTint(OceanTimeOfDay timeOfDay) => timeOfDay switch
    {
    ...
    private static FontAwesomeIcon TimeOfDayIcon(OceanTimeOfDay timeOfDay) => timeOfDay switch
    {
