...
internal sealed class DailiesApp : IPhoneApp
{
    ...
    public void Draw(in PhoneContext context)
    {
        ...
        using (AppSurface.Begin(body))
        {
            ...
            DrawCadence(theme, utcNow, DailyCadence.Weekly, L.Dailies.WeeklyTasks, GameSchedule.NextWeeklyReset(utcNow));
            DrawNotify(theme);
    ...
    private void DrawHero(PhoneTheme theme, DateTime utcNow, float scale)
    {
        ...
        {
            ProgressRing.CenterValue(ringCenter, remaining.ToString(Loc.Culture), $"/ {tracked}", theme.TextStrong, theme.TextMuted, TextStyles.LargeTitle);
        }

        var title = remaining <= 0 ? Loc.T(L.Dailies.AllDone) : Loc.T(L.Dailies.Remaining, remaining);
        Typography.DrawCentered(new Vector2(centerX, ringCenter.Y + radius + 26f * scale), title, theme.TextStrong, TextStyles.Title3);
    ...
    private void DrawCadence(PhoneTheme theme, DateTime utcNow, DailyCadence cadence, LocString sectionTitle, DateTime nextReset)
    {
        var items = DailyCatalog.Items;
        var rowCount = 0;
        for (var index = 0; index < items.Length; index++)
    ...
    private void DrawItemRow(Rect row, PhoneTheme theme, in DailyItem item, in DailyAutoStatus status, DateTime utcNow)
    {
        ...
        {
            Typography.Draw(new Vector2(textLeft, row.Center.Y - 16f * scale), Loc.T(item.Label), theme.TextStrong, TextStyles.Headline);
        ...
        else
        {
            ...
            Typography.Draw(new Vector2(textLeft, row.Center.Y - nameSize.Y * 0.5f), Loc.T(item.Label), theme.TextStrong, TextStyles.Headline);
        }
    ...
    private static void DrawCountValue(Rect row, PhoneTheme theme, in DailyAutoStatus status)
    {
        ...
        Typography.Draw(new Vector2(row.Max.X - valueSize.X, row.Center.Y - valueSize.Y * 0.5f), value, theme.TextStrong, TextStyles.Headline);
    }

    private static void DrawCheckmark(Vector2 center, float radius, bool complete, bool auto, PhoneTheme theme, Vector4 accent)
    {
    ...
    private void DrawNotify(PhoneTheme theme)
    {
        ...
        var notify = SettingsRow.Bool(card.NextRow(), Loc.T(L.Dailies.NotifyReset), configuration.NotifyDailiesReset, theme);
        card.End();
    ...
    private static void SectionHeaderWithCountdown(string title, string countdown, PhoneTheme theme)
    {
        ...
        Typography.Draw(new Vector2(right - countdownSize.X, titleTop + (titleSize.Y - countdownSize.Y) * 0.5f), countdown, theme.TextMuted, TextStyles.Caption1);
    }
