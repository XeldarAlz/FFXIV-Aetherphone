...
internal readonly record struct LinkshellChannel(LinkshellKind Kind, int Slot)
{
    ...
    public static bool TryParse(string key, out LinkshellChannel channel)
    {
        ...
        {
            if (key.StartsWith(CrossWorldPrefix, StringComparison.Ordinal) && int.TryParse(key.AsSpan(CrossWorldPrefix.Length), out var crossWorldSlot))
            {
                channel = new LinkshellChannel(LinkshellKind.CrossWorldLinkshell, crossWorldSlot);
                return true;
            }

            if (key.StartsWith(LocalPrefix, StringComparison.Ordinal) && int.TryParse(key.AsSpan(LocalPrefix.Length), out var localSlot))
            {
