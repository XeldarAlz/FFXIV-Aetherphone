...
internal static unsafe class ActivityReader
{
    public static ActivitySnapshot? Read(GameData gameData)
    {
        ...
        ReadCollection(gameData, playerState, out var mountsOwned, out var mountsTotal, out var minionsOwned,
            out var minionsTotal);
        ReadRetainers(out var retainerCount, out var venturesReady, out var venturesActive);
        return new ActivitySnapshot(gameData.JobName(jobRowId), level, maxLevel, currentExp, neededExp, jobsAtMax,
            jobsTotal, mountsOwned, mountsTotal, minionsOwned, minionsTotal, retainerCount, venturesReady,
            venturesActive);
    }
    ...
    private static void ReadCollection(GameData gameData, PlayerState* playerState, out int mountsOwned,
        out int mountsTotal, out int minionsOwned, out int minionsTotal)
    {
        mountsOwned = 0;
        minionsOwned = 0;
