...
internal sealed class EmulatorCoreProvisioner : IDisposable
{
    ...
    private const string SystemFilesUrl = "https://buildbot.libretro.com/assets/system/PPSSPP.zip";
    private const string SystemFilesFolder = "PPSSPP";
    private const long SystemFilesBytes = 10865689L;
        ...
        StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["geolith_libretro.dll"] = 362530L,
        ["gpsp_libretro.dll"] = 309761L,
    ...
    public bool IsInstalled(EmulatorSystemDefinition system)
    {
        ...
        return !NeedsSystemFiles(system) || SystemFilesInstalled();
    }
    ...
    public long PendingBytes(EmulatorSystemDefinition system)
    {
        ...
        if (!File.Exists(CorePath(system)))
        {
            pending += CoreBytes.TryGetValue(system.CoreFileName, out var bytes) ? bytes : UnknownCoreBytes;
        }

        if (NeedsSystemFiles(system) && !SystemFilesInstalled())
        {
            pending += SystemFilesBytes;
        }
        ...
        CancellationToken token)
    {
        ...
        if (!File.Exists(CorePath(system)))
        {
            ...
            await InstallArchiveAsync(url, coreDirectory, system.CoreFileName, expected,
                    received => onProgress?.Invoke((carried + received) / total), token)
                .ConfigureAwait(false);
            carried += CoreBytes.TryGetValue(system.CoreFileName, out var bytes) ? bytes : UnknownCoreBytes;
        }
        ...
        await InstallArchiveAsync(SystemFilesUrl, systemDirectory, null, null,
                received => onProgress?.Invoke((carried + received) / total), token)
            .ConfigureAwait(false);
        onProgress?.Invoke(1f);
    }
    ...
    private static bool NeedsSystemFiles(EmulatorSystemDefinition system) =>
        system.InputProfile == EmulatorInputProfile.PlayStationPortable;

    private bool SystemFilesInstalled() => Directory.Exists(Path.Combine(systemDirectory, SystemFilesFolder));
