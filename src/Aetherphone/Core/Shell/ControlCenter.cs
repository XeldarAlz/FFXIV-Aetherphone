...
internal sealed class ControlCenter
{
    ...
    public void Draw(Rect screen, PhoneTheme theme, float delta, bool gesturesEnabled)
    {
        ...
        Material.Frosted(dl, new Vector2(screen.Min.X, panelTop), new Vector2(screen.Max.X, panelTop + height), rounding, scale, 1f);
    ...
    private void DrawContents(Rect screen, PhoneTheme theme, float panelTop, float scale, float opacity, bool interactive)
    {
        ...
        var sliderHeight = MathF.Min(150f * scale, screen.Max.Y - 0.18f * screen.Height - sliderTop - (playback.IsActive ? 96f * scale : 8f * scale));
        sliderHeight = MathF.Max(sliderHeight, 96f * scale);
        var sliderWidth = (width - gap) / 2f;

        var brightnessRect = new Rect(new Vector2(left, sliderTop), new Vector2(left + sliderWidth, sliderTop + sliderHeight));
        ...
        {
            DrawNowPlaying(new Rect(new Vector2(left, sliderTop + sliderHeight + 14f * scale), new Vector2(right, sliderTop + sliderHeight + 14f * scale + 78f * scale)), theme, scale, opacity, interactive);
        }
    }
