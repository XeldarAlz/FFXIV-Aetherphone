...
{
    public static void DrawApp(Vector2 center, float size, IPhoneApp app, PhoneTheme theme, float drawScale, float labelAlpha)
    {
        ...
        if (app.BadgeCount > 0)
        {
            DrawBadge(new Vector2(center.X + size * 0.5f - 5f * scale, center.Y - size * 0.5f + 5f * scale), app.BadgeCount, theme, scale);
        }
    }

    public static void DrawFolder(Vector2 center, float size, HomeTile folder, PhoneTheme theme, float drawScale, float labelAlpha, string fallbackName)
    {
        var scale = ImGuiHelpers.GlobalScale;
        var dl = ImGui.GetWindowDrawList();
        var drawHalf = size * 0.5f * drawScale;
    ...
    private static void DrawLabel(Vector2 center, float size, string label, PhoneTheme theme, float scale, float labelAlpha)
    {
        if (labelAlpha <= 0.01f)
        {
            return;
