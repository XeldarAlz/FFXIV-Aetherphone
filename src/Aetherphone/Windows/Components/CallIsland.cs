...
internal sealed class CallIsland
{
    ...
    private static bool Visible(CallView view) => view.State is CallState.Dialing or CallState.Connecting or CallState.Active;
    ...
    private void DrawContent(Rect screen, PhoneTheme theme, INavigator navigation, CallView view)
    {
        ...
        dl.AddRect(bounds.Min, bounds.Max, ImGui.GetColorU32(Palette.WithAlpha(Green, 0.20f + 0.5f * eased)), rounding, ImDrawFlags.RoundCornersAll, 1.5f * scale);
    ...
    private void DrawCompact(ImDrawListPtr dl, Rect compact, float scale, CallView view, float alpha)
    {
        ...
        dl.AddCircleFilled(dotCenter, (3.4f + 1.2f * pulse) * scale, ImGui.GetColorU32(Palette.WithAlpha(Green, alpha)), 16);

        var label = CompactLabel(view);
        var size = Typography.Measure(label, 0.82f);
        Typography.Draw(new Vector2(compact.Max.X - size.X - 11f * scale, compact.Center.Y - size.Y * 0.5f), label, Palette.WithAlpha(Ink, alpha), 0.82f);
    }
    ...
    private bool DrawExpanded(ImDrawListPtr dl, Rect bounds, float scale, PhoneTheme theme, CallView view, float alpha)
    {
        ...
        Typography.DrawCentered(new Vector2(centerX, top + 20f * scale), Truncate(view.PeerLabel, 18), Palette.WithAlpha(theme.TextStrong, alpha), 1.05f);
        Typography.DrawCentered(new Vector2(centerX, top + 42f * scale), CompactLabel(view), Palette.WithAlpha(Green, 0.9f * alpha), 0.82f);
        ...
        if (Button(new Vector2(centerX - 34f * scale, buttonY), 17f * scale, view.Muted ? FontAwesomeIcon.MicrophoneSlash : FontAwesomeIcon.Microphone, muteFill, theme.TextStrong, alpha, active))
        {
            calls.ToggleMute();
            consumed = true;
        }

        if (Button(new Vector2(centerX + 34f * scale, buttonY), 17f * scale, FontAwesomeIcon.PhoneSlash, theme.Danger, Ink, alpha, active))
        {
            calls.Hangup();
            consumed = true;
        }
    ...
    private static bool Button(Vector2 center, float radius, FontAwesomeIcon icon, Vector4 fill, Vector4 ink, float alpha, bool active)
    {
        var dl = ImGui.GetWindowDrawList();
