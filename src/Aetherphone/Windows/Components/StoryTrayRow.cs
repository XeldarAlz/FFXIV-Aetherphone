...
/// <summary>
/// The horizontal row of story rings above a feed. ImGui has no horizontal scroll container in use
...
internal sealed class StoryTrayRow
{
    ...
    public void Draw(Rect row, PhoneTheme theme, AppPalette palette, StoryRingDto[] rings, bool hasOwnStory,
    ...
    private void HandleDrag(Rect row, float scale)
    {
        ...
        if (hovering)
        {
            ...
            if (wheel != 0f)
            {
                offset = Math.Clamp(offset - wheel * 40f * scale, 0f, maxOffset);
            }
        }
        ...
        float scale, Action onAddStory)
    {
        ...
        Typography.DrawCentered(new Vector2(center.X, center.Y + radius + 11f * scale), Loc.T(L.Aethergram.YourStory),
        ...
        StoryRingDto ring, float scale, StoryRingPainter painter, Action<StoryRingDto> onOpenRing)
    {
        ...
        var label = ring.IsMe ? Loc.T(L.Aethergram.YourStory) : SocialIdentity.Name(ring.AuthorDisplayName, ring.AuthorHandle);
