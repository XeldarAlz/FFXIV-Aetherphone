...
internal static class ActivityStatRow
{
    ...
    public static void Draw(Rect row, PhoneTheme theme, Vector4 tint, FontAwesomeIcon icon, string label, string value,
        string? detail = null)
    {
        ...
        {
            Typography.Draw(new Vector2(textLeft, row.Center.Y - 16f * scale), label, theme.TextStrong,
                TextStyles.Headline);
            Typography.Draw(new Vector2(textLeft, row.Center.Y + 5f * scale), detail, theme.TextMuted,
                TextStyles.Footnote);
        }
        else
        {
            var labelSize = Typography.Measure(label, TextStyles.Headline);
            Typography.Draw(new Vector2(textLeft, row.Center.Y - labelSize.Y * 0.5f), label, theme.TextStrong,
                TextStyles.Headline);
        }
        ...
        if (value.Length > 0)
        {
            ...
            Typography.Draw(new Vector2(row.Max.X - valueSize.X, row.Center.Y - valueSize.Y * 0.5f), value, tint, 1.06f,
                FontWeight.SemiBold);
        }
    }

    public static void DrawProgress(Rect row, PhoneTheme theme, Vector4 tint, FontAwesomeIcon icon, string label,
        string value, float fraction, string? detail = null)
    {
        var scale = ImGuiHelpers.GlobalScale;
        var tile = TileSize * scale;
        var tileCenter = new Vector2(row.Min.X + tile * 0.5f, row.Center.Y);
