...
internal sealed class NowPlayingIsland
{
    ...
    private void DrawContent(Rect screen, PhoneTheme theme, INavigator navigation)
    {
        ...
        dl.AddRect(bounds.Min, bounds.Max, ImGui.GetColorU32(Palette.WithAlpha(Accent, 0.14f + 0.46f * eased)),
            rounding, ImDrawFlags.RoundCornersAll, 1.5f * scale);
    ...
    private void DrawPulse(ImDrawListPtr dl, Rect compact, float scale, float collapsedAlpha)
    {
        ...
        dl.AddRect(compact.Min - new Vector2(spread, spread), compact.Max + new Vector2(spread, spread),
            ImGui.GetColorU32(Palette.WithAlpha(Accent, alpha)), rounding, ImDrawFlags.RoundCornersAll, 2.4f * scale);
    }
    ...
    private bool DrawExpanded(ImDrawListPtr dl, Rect bounds, float scale, PhoneTheme theme, float alpha)
    {
        ...
        Typography.Draw(new Vector2(textLeft, top + 18f * scale), Truncate(playback.Title, 16),
            Palette.WithAlpha(theme.TextStrong, alpha), 1.0f, FontWeight.SemiBold);
        Typography.Draw(new Vector2(textLeft, top + 40f * scale), Truncate(playback.Subtitle, 18),
            Palette.WithAlpha(Accent, 0.9f * alpha), 0.8f);
        ...
        if (playback.HasQueue)
        {
            if (TransportButton.Draw(new Vector2(centerX - 46f * scale, controlY), 16f * scale,
                    TransportAction.Previous, Accent, Ink, alpha, active))
            {
                playback.Previous();
                consumed = true;
            }
        ...
        if (TransportButton.Draw(new Vector2(centerX, controlY), 18f * scale, TransportAction.Stop, Accent, Ink, alpha,
                active))
        {
            playback.Stop();
            consumed = true;
        ...
        if (active)
        {
            ...
            playback.Volume = Scrubber.Draw(track, playback.Volume, Accent, Palette.WithAlpha(theme.TextStrong, 0.18f),
                alpha);
        }
