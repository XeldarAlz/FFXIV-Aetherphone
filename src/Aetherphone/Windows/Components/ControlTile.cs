...
internal static class ControlTile
{
        ...
        Vector4 accent, PhoneTheme theme, float opacity, bool interactive)
    {
        ...
        Typography.DrawCentered(dl, new Vector2(rect.Center.X, rect.Max.Y - rect.Height * 0.24f), label, labelColor,
            0.7f, FontWeight.Medium);
        if (hovered)
        {
    ...
    public static float VerticalSlider(ImDrawListPtr dl, Rect rect, float value, FontAwesomeIcon icon, PhoneTheme theme,
        float opacity, bool interactive, out bool released)
    {
        var scale = ImGuiHelpers.GlobalScale;
        var radius = 18f * scale;
        ...
        Squircle.Fill(dl, rect.Min, rect.Max, radius,
            ImGui.GetColorU32(new Vector4(0.96f, 0.96f, 0.97f, 0.92f * opacity)));
        dl.PopClipRect();
        Material.EdgeSquircle(dl, rect.Min, rect.Max, radius, scale, opacity);
        ...
        float opacity, bool interactive)
    {
        ...
        if (selected)
        {
            dl.AddCircle(center, radius + 3f * scale, ImGui.GetColorU32(new Vector4(1f, 1f, 1f, 0.92f * opacity)), 32,
                2f * scale);
        }
